...
def process_recording(recording_uid: str, db: Session) -> dict:
    ...
    print(f"Model result: {result['label']}")
    print(f"Old confidence: {result.get('confidence', 'N/A')}")
    ...
    # Use final_confidence (the new calculated confidence)
    final_confidence = result.get("final_confidence", result.get("confidence", 0.0))
    print(f"Using final_confidence: {final_confidence}")
...
def save_results_and_notify(db: Session, recording_uid: str, results: dict):
    ...
    if tree:
        ...
        tree.current_status = "ONLINE"  # Sensor is working
        print(f"✅ Updated tree {tree.sensor_physical_id} with classification: {results['prediction']}")
    else:
    ...
    print(f"Prediction: {results['prediction']}, Confidence: {results['confidence']}")
    
    # Strict condition for notifications:
    # Only send notification if Infested or Suspicious AND confidence >= threshold
    if results["prediction"] in ["Infested", "Suspicious"]:
        if results["confidence"] >= INFESTATION_THRESHOLD:
            print(f"⚠️ Infestation/Suspicious detected! Triggering notification...")
            _create_infestation_notification(db, recording)
        else:
            print(f"ℹ️ Low confidence ({results['confidence']:.2%}), no notification sent")
    else:
...
def _create_infestation_notification(db: Session, recording):
    ...
    # Get group info
    group = group_crud.get_group_by_id_only(db, tree.group_id) if tree.group_id else None
    ...
    # Send email with notification UID
    email_sent = send_infestation_alert(
        ...
        notification_uid=str(notification.notification_uid)
    )
